...
const TrendingApp = () => {
    ...
    return (
        <div className="w-full space-y-5 relative"> {/* Added relative for button positioning */}
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold">All Trending Apps</h2>
                
                {/* 3. One-sided Navigation Buttons (Top Right) */}
                <div className="flex gap-2">
                    <button className="trending-prev btn btn-circle btn-xs lg:btn-sm btn-outline">❮</button>
                    <button className="trending-next btn btn-circle btn-xs lg:btn-sm btn-outline">❯</button>
                </div>
            </div>
            ...
            <Swiper
                ...
                freeMode
                className="py-4"
                navigation={{ // 5. Linked to custom classes
                ...
                {trendingApp.map((app) => (
                    <SwiperSlide key={app.id}>
                        <Link to={`category/${app.id}`} className="card rounded-xl overflow-hidden">
                            ...
                            <h3 className="text-sm lg:text-lg font-medium mt-2 px-2 truncate">{app.name}</h3>
                        </Link>
                    </SwiperSlide>
                ))}
            </Swiper>
